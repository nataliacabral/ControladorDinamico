...
class SpringSoundObject : SoundObject, Touchable, ModulatorNode
{
    ...
    {
        if (self.springHandle != nil) {
            // Evita que o handle saia do pai
            let topLimit = (self.size.height / 2) - 12
            let bottomLimit = (-self.size.height / 2) + 12
            if (self.springHandle?.position.y >= topLimit &&
                self.springHandle?.physicsBody?.velocity.dy > 0)
            {
                NSLog("Spring top value:%lf", self.springHandle!.currentSoundIntensity())
                self.springHandle?.physicsBody!.velocity.dy *= -1
            }
            else if (self.springHandle?.position.y <= bottomLimit &&
                self.springHandle!.physicsBody!.velocity.dy < 0)
            {
                NSLog("Spring bot value:%lf", self.springHandle!.currentSoundIntensity())
                self.springHandle?.physicsBody!.velocity.dy *= -1
            }
            self.updateSticksPosition()
        }
